...
`config.local.js` 已被 `.gitignore` 忽略，请不要提交真实密钥。
...
- 不要提交 `config.local.js` 或任何真实 API Key。
- The Odds API 和 Football-Data 可能有免费额度限制，请合理设置刷新频率。
- 浏览器 LocalStorage 会保存推演记录、接口缓存和模型校准参数。
- 如果要清除本地推演结果，可在页面中使用清空记录功能，或手动清理浏览器 LocalStorage。
...
`config.local.js` is ignored by Git. Do not commit real API keys.
...
- Do not commit `config.local.js` or real API keys.
- The Odds API and Football-Data may have free-tier limits.
- Browser LocalStorage stores records, API cache, and calibration data.
- Use the in-page clear function or browser LocalStorage tools to reset local records.
